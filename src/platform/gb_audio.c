...
#define AUDIO_CHANNEL_COUNT   2 // Number of stereo channels
#define GB_VOLUME 0.4f;
...
static void CalculateToneVolume(int sampleRate, AudioSample* volume, int gbChannel, int audioChannel);
static void ApplySweep(AudioSample* outBuffer, u16 sampleRate, u16 samplesPerFrame, float prevFreq, AudioSample volume, int audioChannel);
...
static void ApplySweep(AudioSample *outBuffer, u16 sampleRate, u16 samplesPerFrame, float prevFreq, AudioSample volume, int audioChannel)
{
    ...
    for (int regionSampleIndex = 0; regionSampleIndex < samplesPerFrame; regionSampleIndex++, out += AUDIO_CHANNEL_COUNT)
    {
        AudioSample volume;
        ...
        if (relativeWaveStart >= wavePeriod) {
...
static void CalculateToneVolume(int sampleRate, AudioSample* volume, int gbChannel, int audioChannel)
{
    ...
    if (!(gbChannel < AUDIO_CHANNEL_COUNT))
        return;


    float masterVol = (float)((REG_SOUNDCNT_L >> (audioChannel * 4)) & 0x7) / 7.0f;
    *volume = masterVol * ((float)((*envelRegs[gbChannel] >> 12) & 0xF)) / 15.0f * GB_VOLUME;
#else
...
#if 0
    // TEMPORARILY HERE, remove once channel bug is fixed
    if (audioChannel > 0)
        return;
#endif
    ...
    if (!isInactive)
    {
        ...
        *volume = masterVol * dmgVol * (((float)((*envelReg >> 12) & 0xF)) / 15.0f) * GB_VOLUME;
    }
    else
    {
        sEnvelopeStartIndex[gbChannel][audioChannel] = currentSampleIndex[gbChannel][audioChannel];
...
static void MixGBToneChannels(AudioSample* outBuffer, s32 sampleRate, u16 samplesPerFrame)
{
    for (int gbChannel = 0; gbChannel < GB_TONE_CHANNEL_COUNT; gbChannel++)
    {
        ...
        for (int audioChannel = 0; audioChannel < AUDIO_CHANNEL_COUNT; audioChannel++)
        {
            ...
            if (isChannelEnabled) {
                ...
                for (int i = 0; i < samplesPerFrame; i++, out += AUDIO_CHANNEL_COUNT)
                {
                    AudioSample volume;
...
void MixGBChannels(AudioSample* outBuffer, s32 sampleRate, u16 samplesPerFrame)
{
    MixGBToneChannels(outBuffer, sampleRate, samplesPerFrame);
}